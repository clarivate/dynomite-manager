 ...
 * <ul>
 * <li>Archaius: Excellent option for enterprise deployments as it provides
 * dynamic properties (i.e. configuration management)
 * <li>Environment variables: Localized configuration passed in via environment
 * variables
 * <li>Java properties: Localized configuration passed in via the command line
 * in an init scrip
 * </ul>
 */
@Singleton
public class DynomiteManagerConfiguration implements IConfiguration {
	...
	public static final String DATASTORE_PREFIX = "datastore"; // Storage engine
																// (aka backend)
	...
	// The cluster name is used as the default AWS Security Group name, if SG
	// name is null.
	...
	// milliseconds
	private static final String CONFIG_DYNOMITE_GOSSIP_INTERVAL = DYNOMITE_PROPS + ".gossip.interval"; // in
																										// ms
	...
	// The max percentage of system memory to be allocated to the backend data
	// storage engine (ex. Redis, ARDB).
	...
	private static final String CONFIG_ARDB_ROCKSDB_MAX_WRITE_BUFFER_NUMBER = ARDB_ROCKSDB_PROPS
			+ ".max.write.buffer.number";
	private static final String CONFIG_ARDB_ROCKSDB_MIN_MEMTABLES_TO_MERGE = ARDB_ROCKSDB_PROPS
	...
	public DynomiteManagerConfiguration(ICredential provider, IConfigSource configSource,
			InstanceDataRetriever retriever, InstanceEnvIdentity insEnvIdentity, IStorageProxy storageProxy) {
	...
	public String getDynomiteYaml() {
		...
		// If a user sets a relative path to dynomite.yaml then we need to
		// prepend the Dynomite installation directory
	...
	public boolean isForceWarm() {
		return getBooleanProperty("DM_FORCE_WARM", CONFIG_DYNO_WARM_FORCE, false);
	}
	...
	public boolean isRedisAofEnabled() {
		// Call getRedisPersistenceType() only once to simplify testing. An
